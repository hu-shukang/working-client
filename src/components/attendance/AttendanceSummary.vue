<template>
  ...
  <div class="container">
    ...
    <div class="item">{{ remotelyMoney }}</div>
...
const remotelyDaliyMoney = 400;
const remotelyMoney = computed(() => {
  const count = props.data.remotelyCount;
  return `${count}日 × ${stringUtil.formatMoney(
    remotelyDaliyMoney,
  )}/日 = ${stringUtil.formatMoney(count * remotelyDaliyMoney)}`;
});
</script>
